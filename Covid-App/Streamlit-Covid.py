...
def sidebar():
    selectDataSource = st.sidebar.selectbox('Choose your preferred DataSource',('RKI','WHO'))
    if selectDataSource == 'RKI':
        # load csv with RKI data
        pass
    if selectDataSource == 'WHO':
        # load csv with WHO Data
        df = pd.read_csv('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv', error_bad_lines=False)
        st.markdown('Confirmed Cases')
        st.dataframe(df) # just printing df - later i want to produce a chart
        pass
